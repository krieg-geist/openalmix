...
void SamplePlayer::Play() {
  ...
  // alSourcei(p_Source, AL_LOOPING, AL_TRUE);
  ...
  alSourcef(p_Source, AL_GAIN, 0.0);
}

void SamplePlayer::Pause() { alSourcePause(p_Source); }

void SamplePlayer::Stop() { alSourceStop(p_Source); }

void SamplePlayer::Resume() { alSourcePlay(p_Source); }

void SamplePlayer::SetVol(int vol) {
  if (0 <= vol || vol <= 127)
  {
    if (vol > 3)
    {
      float fVol = (float)vol / 127;
      alSourcef(p_Source, AL_GAIN, fVol);
    }
    else
      alSourcef(p_Source, AL_GAIN, 0.0);
  }
  else
    throw std::runtime_error("Invalid vol");
}

void SamplePlayer::SetPan(float pan) {
  if (-1.0 <= pan && pan <= 1.0) {
    float sourcePosAL[] = {pan, 0.0f, 0.0f};
    alSourcefv(p_Source, AL_POSITION, sourcePosAL);
  } else
    throw std::runtime_error("Invalid pan");
}

void SamplePlayer::UpdateBufferStream() {
...
ALint SamplePlayer::getSource() { return p_Source; }
...
SamplePlayer::SamplePlayer(const char *filename) {
